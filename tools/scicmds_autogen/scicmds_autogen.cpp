...
int main()
{
    ...
    for (auto& macro : macros) {
        using namespace std;
        auto& name = macro.first;
        if (name._Starts_with("SCK_"sv)) {
            scikeys.push_back(std::make_pair(name.substr(sizeof("SCK_") - 1), macro.second));
        }
        else if (name._Starts_with("VK_F"sv) && ::isdigit(name[sizeof("VK_F") - 1]))
        {
            scikeys.push_back(std::make_pair(name.substr(sizeof("VK_") - 1), macro.second));
        }
    }
    ...
    ss << "-- This file is generated by scicmds_autogen v1.0.0 automatically.\n";
    ss << "local scicmds = {\n";
