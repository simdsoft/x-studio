...
struct Node3DOption FLATBUFFERS_FINAL_CLASS : private flatbuffers::Table {
  ...
  const Vector3 *position3D() const {
    return GetStruct<const Vector3 *>(VT_POSITION3D);
  }
  const Vector3 *rotation3D() const {
    return GetStruct<const Vector3 *>(VT_ROTATION3D);
  }
  const Vector3 *scale3D() const {
    return GetStruct<const Vector3 *>(VT_SCALE3D);
  }
...
struct Node3DOptionBuilder {
  ...
  void add_position3D(const Vector3 *position3D) {
    fbb_.AddStruct(Node3DOption::VT_POSITION3D, position3D);
  }
  void add_rotation3D(const Vector3 *rotation3D) {
    fbb_.AddStruct(Node3DOption::VT_ROTATION3D, rotation3D);
  }
  void add_scale3D(const Vector3 *scale3D) {
